#!/usr/bin/env python3
"""
Simple script to create a Titanic-themed PDF using basic text file approach.
"""

def create_titanic_text_file():
    """Create a Titanic-themed text file that can be converted to PDF."""
    
    content = """The RMS Titanic: A Historical Overview
...
Notable Passengers
Some of the most famous passengers included:
...
- Captain Edward Smith, the ship's captain
...
- Engines: Two four-cylinder triple-expansion steam engines
- Propellers: Three (two three-bladed, one four-bladed)
- Boilers: 29 double-ended and 5 single-ended boilers
- Funnels: 4 (only 3 were functional)

Safety Features
The Titanic was equipped with several safety features for its time:
- Watertight compartments (16 total)
- Double bottom hull
- Wireless telegraph system
- Lifeboats (though only 20 for 2,224 people)
- Watertight doors
- Emergency lighting system

The Lifeboats
The Titanic carried 20 lifeboats with a total capacity of 1,178 people, 
which was more than required by the British Board of Trade regulations 
at the time. However, this was only enough for about half of the people 
on board. The lifeboats were launched with far fewer people than they 
could carry, as many passengers were reluctant to leave the "unsinkable" ship.

Rescue Operations
The RMS Carpathia, a Cunard Line ship, was the first to respond to 
the Titanic's distress signals. She arrived at the scene around 4:00 AM 
and rescued 705 survivors from the lifeboats. The survivors were taken 
to New York, where they arrived on April 18, 1912.

Investigation and Findings
Both the United States and Great Britain conducted investigations into 
the disaster. The inquiries found that the ship was traveling too fast 
for the conditions, the lookouts had no binoculars, and there were 
insufficient lifeboats. These findings led to significant changes in 
maritime safety regulations.

Modern Discoveries
Since the wreck's discovery in 1985, numerous expeditions have been 
conducted to study and document the site. The ship is slowly deteriorating 
due to deep-sea conditions, including salt water corrosion and the 
activities of deep-sea organisms. Many artifacts have been recovered 
and are displayed in museums around the world.
"""
    
    # Create text file
    filename = "titanic_history.txt"
    with open(filename, 'w', encoding='utf-8') as f:
        f.write(content)
    
    print(f"✅ Created Titanic text file: {filename}")
    print("📝 You can convert this to PDF using online tools or print to PDF")
    return filename

if __name__ == "__main__":
    create_titanic_text_file()
